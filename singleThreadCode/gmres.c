...
void gmres(int n, double* A, int lda, double* x, double* b, double* LU,
...
#ifdef HPL_CALL_MKL
    double norm_b = LAPACKE_dlange(LAPACK_COL_MAJOR,'F',n,1,b,n);
#else
...
#ifdef HPL_CALL_MKL
    cblas_dgemv(CblasColMajor,CblasNoTrans, n, n, -1.0, A, lda, x, 1, 1.0, r, 1);
    cblas_dtrsm (CblasColMajor,CblasLeft,CblasLower,CblasNoTrans,CblasUnit , n, 1, 1.0, LU, ldlu, r, n);   
    cblas_dtrsm (CblasColMajor,CblasLeft,CblasUpper,CblasNoTrans,CblasNonUnit, n, 1, 1.0, LU, ldlu, r, n);   
    error = LAPACKE_dlange(LAPACK_COL_MAJOR,'F',n,1,r,n) / norm_b;
#else
    ...
    if (error < tol) {
        ...
        // V0 = r; s[0] = |r|;
#ifdef HPL_CALL_MKL
        double norm_r = LAPACKE_dlange(LAPACK_COL_MAJOR,'F',n,1,r,n);
#else
        ...
        for (i = 0; i < m; i++) {
            // w = U \  (L \ (A * Vi))
#ifdef HPL_CALL_MKL
            cblas_dgemv(CblasColMajor,CblasNoTrans, n, n, 1.0, A, lda, V + i * n, 1, 0.0, w, 1);
            cblas_dtrsm (CblasColMajor,CblasLeft,CblasLower,CblasNoTrans,CblasUnit , n, 1, 1.0, LU, ldlu, w, n);
            cblas_dtrsm (CblasColMajor,CblasLeft,CblasUpper,CblasNoTrans,CblasNonUnit,n, 1, 1.0, LU, ldlu, w, n);
#else
...
#ifdef HPL_CALL_MKL
            H(i + 1, i) = LAPACKE_dlange(LAPACK_COL_MAJOR,'F',n,1,w,n);         
#else
            ...
            if (error <= tol) {
...
#ifdef  HPL_CALL_MKL
                cblas_dtrsm (CblasColMajor,CblasLeft,CblasUpper,CblasNoTrans,CblasNonUnit, i + 1, 1, 1.0, H, m + 1, w, n);
                cblas_dgemv(CblasColMajor,CblasNoTrans,n, i + 1, 1.0, V, n, w, 1, 1.0, x, 1);
#else
                ...
                double error = LAPACKE_dlange(LAPACK_COL_MAJOR,'I', n, 1, r, n) /
